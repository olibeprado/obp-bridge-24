        ...
        while self.running:
            try:
                ...
                self.save_to_supabase({
                    "reward": reward, "state": state, "sinal": sinal,
    ...
    def do_GET(self):
        if self.path in ('/health', '/'):
            ...
            response = {
                ...
                'generation': _singularity_ref[0].generation if _singularity_ref[0] else 0,
                'trades': _singularity_ref[0].trades if _singularity_ref[0] else 0,
                'wins': _singularity_ref[0].wins if _singularity_ref[0] else 0,
                'estado': _singularity_ref[0].estado.value if _singularity_ref[0] else 'N/A',
...
if __name__ == "__main__":
    ...
    try:
        ...
        print(f"\n[{datetime.now().strftime('%H:%M:%S')}] 🛑 Encerrada.")
    except Exception as e:
        print(f"\n[{datetime.now().strftime('%H:%M:%S')}] ❌ Health server erro: {e}")
        print(f"[{datetime.now().strftime('%H:%M:%S')}] 🔄 Mantendo vivo...")
        while singularity.running:
            time.sleep(60)
